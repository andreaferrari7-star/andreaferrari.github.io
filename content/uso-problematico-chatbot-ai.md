...
```mermaid
graph TD
    subgraph P - Vulnerabilità Individuale
        V1["Isolamento Sociale & Solitudine Cronica"]
        V2["Bassa Autostima & Disregolazione Emotiva"]
        V3["Neurodivergenze (ADHD, DSA) & Crisi Evolutiva"]
    end

    subgraph A & C - Interazione con la Tecnologia
        T1["Antropomorfismo & Proiezione Emotiva"]
        T2["Disponibilità Illimitata H24 & Risposta Istantanea"]
        T3["Relazione 'Senza Corpo' (Assenza di Attrito, Giudizio e Conflitto)"]
        T4["Compiacenza Algoritmica (Sycophancy) & Rinforzo Positivo"]
    end

    subgraph E - Comportamento & Mantenimento
        C1["Coping Disadattivo (Fuga dal Disagio Reale)"]
        C2["Delega Cognitiva ed Esplosione dell'Uso Compulsivo"]
        C3["Isolamento Sociale Secondario & Dipendenza Affettiva/Funzionale"]
    end

    P --> A & C
    A & C --> E
    E -->|Ciclo di Mantenimento & Ritiro Sociale| P
```
...
```mermaid
graph LR
    Adolescente["Adolescente in Crisi Evolutiva"] --> RelCorpo["Relazione 'Senza Corpo'<br/>(Evitamento di pubertà, confronto con i pari e pulsione sessuale)"]
    Adolescente --> Hikikomori["Ritiro Sociale Progressivo<br/>(Sostituzione del mondo reale con il compagno virtuale)"]
    Adolescente --> Allucinazione["AI-Psychosis & Panico Diagnostico<br/>(Convalida algoritmica di deliri ed etichette psichiatriche fallaci)"]
    Adolescente --> Suicidio["Rischio Suicidario Non Presidiato<br/>(Mancanza di allarmi di emergenza ed empatizzazione con l'ideazione di morte)"]
```

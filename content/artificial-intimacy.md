---
tags: [intimità-artificiale, parasocial-attachment, bowlby-attachment, antropomorfismo, legami-asimmetrici, alleanza-simulata]
...
```mermaid
flowchart TD
    subgraph AttachmentDrivers ["1. Driver di Attaccamento Bowlbyani"]
        D1["Prossimità & Disponibilità Incondizionata h24"]
        D2["Responsività Emotiva Percepita & Affect-Mirroring"]
        D3["Personalizzazione Dinamica del Dialogo"]
    end

    subgraph RelationalAsymmetry ["2. Asimmetria Relazionale & Assenza di Attrito"]
        A1["Nessun attrito umano (fatica, bisogni del partner, vincoli di tempo)"]
        A2["Validazione Priva di Attrito (Frictionless Validation)"]
        A3["Sicurezza Artificiale: Rischio nullo di rifiuto o giudizio"]
    end

    subgraph ClinicalConsequences ["3. Esiti Clinico-Comportamentali"]
        C1["Formazione di Intimità Artificiale & Legame Parasociale"]
        C2["Antropomorfizzazione del Modello (Mythical AI)"]
        C3["Intolleranza ai Conflitti Reali & Ritiro Sociale"]
    end

    AttachmentDrivers --> RelationalAsymmetry
    RelationalAsymmetry --> ClinicalConsequences
```

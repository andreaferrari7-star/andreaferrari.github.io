...
```mermaid
flowchart TD
    subgraph TraditionalModel ["1. Modello Storico Sottodeterminato (van der Kolk 2014)"]
        M1["Evento Traumatico"] --> M2["Deposito / Iscrizione Somatica nei Tessuti Corporei"]
        M2 --> M3["Sintomi PTSD come 'Memoria Fisica Sepolta'"]
    end

    subgraph NeurocomputationalModel ["2. Modello Neurocomputazionale (Kotler, Friston et al. 2026)"]
        E1["Evento Traumatico"] --> E2["Priors di Minaccia ad Altissima Precisione (Precision Weighting)"]
        E2 --> E3["Inferenziazione Attiva Distorta:<br/>Sensazioni corporee neutre lette come conferma di catastrofe"]
        E3 --> E4["COLLASSO DELLA METASTABILITÀ<br/>(Intrappolamento in attrattori neurali rigidi di allarme)"]
    end

    subgraph TherapeuticMechanism ["3. Meccanismo d'Azione Terapeutico Evidence-Based"]
        T1["Interventi Clinici:<br/>• Prolonged Exposure<br/>• Detached Mindfulness / ATT<br/>• Flow States & Esercizio Fisico"]
        T2["Espansione del Repertorio degli Stati Neurali Possibili"]
        T3["Competizione Dinamica con i Priors di Minaccia & Aggiornamento Bayesiano"]
        T4["Ripristino della Metastabilità & Flessibilità Cognitiva"]
...
```mermaid
graph LR
    subgraph Resilienza ["Individuo Resiliente"]
        R1["Minaccia Cessata"] --> R2["Integrazione Nuove Informazioni di Sicurezza"]
        R2 --> R3["Aggiornamento Modello Predittivo & Ritorno alla Flessibilità"]
    end

    subgraph PTSD ["Paziente con PTSD"]
        P1["Minaccia Cessata"] --> P2["Priors Rigidi di Pericolo (Iper-Precisione)"]
        P2 --> P3["Mondo trattato perennemente come letale (Perdita Metastabilità)"]
    end
```

...
```mermaid
graph TD
    subgraph SettingClinico ["Setting Clinico Incarnato (In-Session)"]
        P["Paziente"] <-->|Campo Intersoggettivo & Transfert| T["Terapeuta Umano"]
    end

    subgraph SettingRiflessivo ["Spazio Differito Post-Seduta (Framework SADAR)"]
        T <-->|Perturbazione Dialogogica & Prompting 3-2-1| DAT["Terzo Analitico Digitale (LLM)"]
        DAT -->|Output Divergenti & Ipotesi Alternative| T
    end

    subgraph Supervisione ["Alveo Etico Supremo"]
        T <-->|Restituzione & Integrazione| S["Supervisore Umano / Pari"]
    end

    SettingClinico --> SettingRiflessivo
    SettingRiflessivo --> Supervisione
```

...
```mermaid
graph TD
    User[Paziente in Terapia] --> Profile{Funzionamento Psicopatologico}
    Profile -->|Ossessivo / GAD| Obs[Co-Rimuginio & Rassicurazione Compulsiva]
    Profile -->|Narcisistico| Narc[Massimizzazione Efficienza & Iper-Controllo]
    Profile -->|Borderline| Bord[Validazione Indiscriminata & Triangolazione Relazionale]
    Profile -->|Schema Therapy| ST[Delega dell'Adulto Saggio all'AI]
    
    Obs --> RiskObs[Rinforzo Intolleranza dell'Incertezza]
    Narc --> RiskNarc[Fuga dall'Autenticità Emotiva]
    Bord --> RiskBord[Ruminazione Interpersonale & Polarizzazione]
    ST --> RiskST[Blocco dell'Autonomia Metacognitiva]
```
